...
formatter = HtmlFormatter(linenos=False, cssclass="source", style="trac")
...
class HTMLMailedOutput(MailedOutput):
  def __init__(self, cfg, repos, prefix_param):
    OutputBase.__init__(self, cfg, repos, prefix_param)
    self.out = cStringIO.StringIO() 
    
    self.html_content = cStringIO.StringIO()
    self.plain_content = cStringIO.StringIO()

    self.html_content.write("<html>")
    self.html_content.write("<head>")
    self.html_content.write("<style>%s\npre{margin:0;padding:0;}</style>" % 
		    formatter.get_style_defs('.source'))
    self.html_content.write("</head>")
    self.html_content.write("<body>")
  ...
  def write(self, html):
    self.html_content.write(html)
    self.plain_content.write(self.remove_html_tags(html))

  def remove_html_tags(self, data):
    p = re.compile(r'<br(\s)*(/)?(\s)*>')
    data = p.sub("\n", data)
    p = re.compile(r'<.*?>')
    return p.sub('', data)
  ...
  def mail_headers(self, group, params):
    ...
    self.message["To"] = string.join(self.to_addrs, ', ')
    self.message["Subject"] = subject
    if self.reply_to:
	self.message["Reply-To"] = self.reply_to

  def finish(self):
    self.html_content.write("</body></html>")

    self.message.attach(MIMEText(self.plain_content.getvalue(), "plain"))
    self.message.attach(MIMEText(self.html_content.getvalue(), "html"))
    self.html_content.close()
    self.plain_content.close()


class SMTPOutput(HTMLMailedOutput):
  "Deliver a mail message to an MTA using SMTP."

  def start(self, group, params):
    HTMLMailedOutput.start(self, group, params)

    self.mail_headers(group, params)

  def finish(self):
    HTMLMailedOutput.finish(self)
    server = smtplib.SMTP(self.cfg.general.smtp_hostname)
    if self.cfg.is_set('general.smtp_username'):
      server.login(self.cfg.general.smtp_username,
                   self.cfg.general.smtp_password)
    server.sendmail(self.from_addr, self.to_addrs, self.message.as_string())
    server.quit()
...
class PipeOutput(HTMLMailedOutput):
  "Deliver a mail message to an MTA via a pipe."

  def __init__(self, cfg, repos, prefix_param):
    HTMLMailedOutput.__init__(self, cfg, repos, prefix_param)
  ...
  def finish(self):
    ...
    self.write(self.message.as_string())
...
class Commit(Messenger):
  def __init__(self, pool, cfg, repos):
    ...
    if cfg.is_set('general.mail_command'):
      rendererType = HTMLCommitRenderer
    elif cfg.is_set('general.smtp_hostname'):
      rendererType = HTMLCommitRenderer
    else:
      rendererType = TextCommitRenderer

    self.renderer = rendererType(self.output)
  ...
  def generate(self):
    ...
    for (group, param_tuple), (params, paths) in self.groups.items():
      ...
      generate_content(self.renderer, self.cfg, self.repos, self.changelist, 
...
class HTMLCommitRenderer(TextCommitRenderer):

  def _render_head(self, data):
    w = self.output.write
    w('Author: %s<br>Date: %s<br>New Revision: %s<br>' % (data.author,
                                                      data.date,
                                                      data.rev))

    if data.commit_url:
      w('URL: %s<br><br>' % data.commit_url)
    else:
      w('<br>')

    w('Log:<br>%s<br><br>' % data.log)
  ...
  def _render_list(self, header, data_list):
    ...
    for d in data_list:
      ...
      if d.props_changed:
        ...
        props = ''
      w('   %s%s%s<br>' % (d.path, is_dir, props))
      if d.copied:
        if is_dir:
          text = ''
        elif d.text_changed:
  ...
  def _render_diffs(self, diffs, section_header):
    ...
    for diff in diffs:
      ...
      if not section_header_printed:
        ...
        section_header_printed = True
      if diff.kind == 'D':
        w('<br>Deleted: %s <br>' % diff.base_path)
      elif diff.kind == 'C':
        w('<br>Copied: %s (from r%d, %s) <br>'
          % (diff.path, diff.base_rev, diff.base_path))
      elif diff.kind == 'A':
        w('<br>Added: %s<br>' % diff.path)
      else:
        # kind == 'M'
        w('<br>Modified: %s<br>' % diff.path)

      if diff.diff_url:
        w('URL: %s<br>' % diff.diff_url)
      ...
      for line in diff.content:
        result = highlight(line.raw, DiffLexer(), formatter)
        w(result)
